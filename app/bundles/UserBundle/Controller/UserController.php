...
class UserController extends FormController
{
    ...
    public function applicantsAction($page = 1)
    {
        if (!$this->get('mautic.security')->isGranted('user:users:view')) {
            return $this->accessDenied();
        }
        ...
        return $this->delegateView([
            'viewParameters' => $parameters,
            'contentTemplate' => 'MauticUserBundle:User:applicants.html.php',
            'passthroughVars' => [
                'route' => $this->generateUrl('mautic_user_index', ['page' => $page]),
                'mauticContent' => 'user',
            ],
     ...
     * Active a user
     */
    public function activeAction($objectId, $ignorePost = false)
    {
        if (!$this->get('mautic.security')->isGranted('user:users:edit')) {
            return $this->accessDenied();
        }
    ...
    public function addFieldAction(Request $request)
    {
        ...
        $form = $model->createForm($field, $this->get('form.factory'), $this->generateUrl('mautic_user_add_field'), [
            'object_choices' => [
        ...
        return $this->delegateView(
            [
                ...
                'contentTemplate' => 'MauticUserBundle:User:addField.html.php',
                'passthroughVars' => [
