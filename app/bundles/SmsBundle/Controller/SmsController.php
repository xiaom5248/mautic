...
class SmsController extends AbstractFormController
{
    ...
    public function newAction($entity = null)
    {
        ...
        $action = $this->generateUrl('mautic_sms_action', ['objectAction' => 'new', 'objectId' => $objectId]);
...
//        }
     ...
     * @return JsonResponse|Response
     */
    public function patchSendAction($objectId)
    {
        /** @var \Mautic\SmsBundle\Model\SmsModel $model */
        $model    = $this->getModel('sms');
        $sms      = $model->getEntity($objectId);
        ...
        $segRepo = $segmentModel->getRepository();
        $seg_lists = $segRepo->getLists();
        ...
        $action = $this->generateUrl('mautic_sms_action',['objectAction'    =>  'patchSend','objectId'  =>  $objectId]);


        $time = new \DateTime();
        $form = $this->get('form.factory')->create(SmsPatchSendType::class, ['sendTime' => $time], ['action' => $action]);
        ...
        if($this->request->getMethod() == 'POST') {
            ...
            if(!$isCancelled && $isValid) {
                $segment = $form['lists']->getData();
                $sendTime = $form['sendTime']->getData();
                ...
                if($time>=$sendTime){
                    ...
                    $event->setEventType("action");
                    $event->setOrder(0);
                    $event->setProperties(["segment_id" => $segment,'send' => false]);
                    $event->setTriggerDate($sendTime);
                    $event->setTriggerInterval(1);
                    $event->setTriggerIntervalUnit("d");
                    $event->setTriggerMode("date");
                ...
                $sms->setGroupSendCount($sms->getGroupSendCount()+1);
                $model->saveEntity($sms);
            }
        ...
        if ($sms !== null && $security->hasEntityAccess('sms:smses:viewown', 'sms:smses:viewother',$sms->getCreatedBy())) {
            return $this->delegateView([
                ...
                'contentTemplate' => 'MauticSmsBundle:Sms:patch_send.html.php',
                'passthroughVars' => [
                    'activeLink'    => '#mautic_sms_index',
                    'mauticContent' => 'sms',
                    'updateSelect'  => InputHelper::clean($this->request->query->get('updateSelect')),
    ...
    public function shortAction($objectId)
    {
        $url = $this->request->get("url");
        $url = urlencode($url);
        $request_url = "http://api.t.sina.com.cn/short_url/shorten.json?source=3946972952&url_long=$url";

        $connector = HttpFactory::getHttp();
        $data   = $connector->post($request_url, [], [], 10);
        $info = json_decode($data->body);
